...
ImageWatcher.prototype.setup = function (config) {
    ...
    self.folders = {}; //Folder object with images paths grouped by folder
    ...
    function readFile(fileName, next) {
        ...
        function processFile(checksum) {
            ...
            self.folders[fileDir] = _.chain(self.folders[fileDir] || []).push(filePath).sort().uniq();
            if (!config.processing) {
    ...
    function updated() {
        self.emit('update', self.images, self.folders);
    }
    ...
    function watchFiles() {
        self.emit('ready', self.images, self.folders);
